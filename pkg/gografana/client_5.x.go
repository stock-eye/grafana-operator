...
func (gc *GrafanaClient_5_0) GetDashboardsByFolderId(folderId, page int) ([]Board, error) {
	urlPath := fmt.Sprintf("%s/api/search?folderIds=%d&page=%d", gc.basicAddress, folderId, page)
	req, err := http.NewRequest("GET", urlPath, nil)
	if err != nil {
		return nil, err
	}
